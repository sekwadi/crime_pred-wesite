...
if(isset($_POST['delete_officer'])){
  $id = $_POST['delete_officer'];
 
  $sql = "DELETE FROM officer WHERE officer_id = '$id'";
  $results = mysqli_query($conn, $sql);
  header('location:./admin_officer.php?msg=Officer deleted successfully');
...
if(isset($_POST['edit_officer'])){
  ...
  $_SESSION['del_officer_id'] = $_POST['edit_officer'];

  echo $_SESSION['del_officer_id'];
  header('location:./admin_edit_officer.php');
}
...
<head>
    ...
    <title>Admin Officer | Crime Prediction</title>
...
<section style="font-size: 3rem; padding:1rem; background-color:#0d6efd; color:white; ">
    <h1 style="font-weight:600">Dashbord > Officer</h1>
</section>
...
<button class="btn btn-outline-primary" ><a href="./admin_add_officer.php">Add new officer</a> </button>


<br><br>

<div class="table-responsive-lg" style="padding:0rem 2rem">
...
<?php 
while($officer_info_row = mysqli_fetch_array($officer_info)){
?>

    <tr>
      <th scope="row">#</th>
      <td><?php print_r($officer_info_row['officer_id']) ?></td>
      <td><?php print_r($officer_info_row['fname']) ?></td>
      <td><?php print_r($officer_info_row['lname']) ?></td>
   
  
      <td><?php print_r($officer_info_row['email']) ?></td>
      <td><?php print_r($officer_info_row['phone']) ?></td>
  

      <td>
    <form action="" method="post">
        <button name="edit_officer" value="<?php print_r($officer_info_row['officer_id']) ?>" style="width:4rem; margin:1px" class="btn btn-primary btn-sm" data-target="modal-js-example11">Edit</button>
     
    </form>
    
     <form action="./admin_officer.php" method="post">
         <button name="delete_officer" value="<?php print_r($officer_info_row['officer_id']) ?>" style="width:4rem; margin:1px" class="btn btn-danger btn-sm">Delete</button>
     </form>
     </td>
