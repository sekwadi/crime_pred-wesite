...
class Graph:
    ...
    def uniform_cost_search(self, start, goal):
        found, fringe, visited, came_from, cost_so_far = False, [
            (0, start)], set([start]), {start: None}, {start: 0}
...
while True:
    edge = input("Enter an edge as the source destination:- ")
    if not edge:
        break
    source, dest, cost = edge.split()
    graph.add_edge(source, dest, int(cost))

start = input("enter the source node to search from:- ")
